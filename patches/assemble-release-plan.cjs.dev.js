          ...
          for (const { depType, versionRange } of dependencyVersionRanges) {
            if (
              ...
              if (
                ...
                (config.___experimentalUnsafeOptions_WILL_CHANGE_IN_PATCH.updateInternalDependents === 'always' ||
                  !semver__default['default'].satisfies(
                    incrementVersion(nextRelease, preInfo), // to deal with a * versionRange that comes from workspace:* dependencies as the wildcard will match anything
...
function applyLinks(releases, packagesByName, linked) {
  ...
  for (let linkedPackages of linked) {
    ...
    if (!highestVersion || !highestReleaseType)
      throw new Error(`Large internal changesets error in calculating linked versions. Please contact the maintainers`); // Finally, we update the packages so all of them are on the highest version
